...
DEPEND_COMMON="clvm? ( cman? ( =sys-cluster/cman-3* ) corosync? ( sys-cluster/corosync ) openais? ( sys-cluster/openais ) >=sys-cluster/dlm-4 )
...
src_prepare() {
	...
	epatch "${MYDIR}"/${PN}-2.02.129-example.conf.in.patch
	# Funtoo change. FL-1755. Do not probe cdrom by lvm
	epatch "${MYDIR}"/${PN}-2.02.145-no-cdrom-probe.patch
	...
	epatch "${MYDIR}"/${PN}-2.02.139-dynamic-static-ldflags.patch #332905
	epatch "${MYDIR}"/${PN}-2.02.129-static-pkgconfig-libs.patch #370217, #439414 + blkid
	epatch "${MYDIR}"/${PN}-2.02.130-pthread-pkgconfig.patch #492450
	epatch "${MYDIR}"/${PN}-2.02.145-mkdev.patch #580062
